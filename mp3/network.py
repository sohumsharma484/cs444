...
class Anchors(nn.Module):
    def __init__(
        ...
        """
        self.stride = stride
        sizes = torch.Tensor(sizes).reshape((len(sizes), 1))
        aspect_ratios = torch.Tensor(aspect_ratios).reshape((1, len(aspect_ratios)))
        # w^2 * aspect_ratio = (size*stride)^2
        # w = (size*stride)/sqrt(aspect_ratio)
        w = (sizes * self.stride / torch.sqrt(aspect_ratios))
        h = aspect_ratios * w
        w, h = w.ravel(), h.ravel()
        self.anchor_offsets = torch.stack([-w/2, -h/2, w/2, h/2]).T.reshape(-1,4,1,1)
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            to get tensors for x1, x2, y1, and y2 for all the anchor boxes. Then you should simply stack those tensors together, reshape them to match the expected output
            size and use torch.repeat to repeat that tensor B times across the batch dimension.
            Your final code should be fully vectorized and not have any for loops.
            Also make sure that when you create a tensor you put it on the same device that x is on.
        """
        b, _, h, w = x.shape
        arranged = torch.stack(torch.meshgrid(torch.arange(0, w*self.stride, self.stride), torch.arange(0, h*self.stride, self.stride), indexing='xy')).reshape(2, h, w)
        # print(arranged)
        anchors = arranged.repeat(2, 1, 1) + self.anchor_offsets
        # print(anchors[0])
        return anchors.reshape(-1, h, w).repeat(b,1,1,1).to(x.device)
